...
const LIB_DIR = 'lib';
const SRC_CLASSES_PATH = path.join(ROOT, 'src', 'classes');

function classToLibPaths(className: string) {
  return {
    types: `./${LIB_DIR}/types/classes/${className}/index.d.ts`,
    import: `./${LIB_DIR}/esm/classes/${className}/index.js`,
    require: `./${LIB_DIR}/cjs/classes/${className}/index.js`,
  };
}
...
function looksLikeClassName(name: string) {
  return /^[A-Z0-9]+/.test(name) && !/^Base[A-Z]/.test(name);
}

async function listClasses(srcClassesDir: string): Promise<string[]> {
  const items = await fs.promises.readdir(srcClassesDir, {
    withFileTypes: true,
  });
  ...
  for (const it of items) {
    if (it.isDirectory()) {
      ...
      if (looksLikeClassName(dirName)) {
        classSet.add(dirName);
      }
    }
  }
...
function buildExportsMap(
  ...
  // Subpaths for each class: "./className"
  for (const className of classes) {
    map[`./${className}`] = classToLibPaths(className);
  }
...
async function main() {
  ...
  await ensureExists(SRC_CLASSES_PATH);

  const pkgPath = path.join(ROOT, 'package.json');
  const pkg = await readJson<PackageJson>(pkgPath);

  const classes = await listClasses(SRC_CLASSES_PATH);
  if (classes.length === 0) {
    console.warn('⚠️  Classes not found in src/classes. Nothing to export.');
  } else {
    console.log(`✔ Found classes: ${classes.join(', ')}`);
  }

  const exportsMap = buildExportsMap(classes);
  ...
  console.log(`  - Classes: ${LIB_DIR}/<className>/index.{js,cjs,d.ts}`);
}
